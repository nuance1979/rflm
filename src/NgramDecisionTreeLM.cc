...
NgramDecisionTreeLM::NgramDecisionTreeLM(Vocab &vocab, 
					 NgramDecisionTree<NgramCount> &dt,
					 Discount &dist)
  : Ngram(vocab, dt.getOrder()), tree(&dt), estimated(false)
{
  tree->setDiscount(dist);
}

NgramDecisionTreeLM::~NgramDecisionTreeLM()
{
}

LogP
NgramDecisionTreeLM::wordProb(VocabIndex word, const VocabIndex *context)
{
  assert(tree);

  return estimated ? Ngram::wordProb(word, context) 
    : tree->wordProb(word, context);
}

Boolean
NgramDecisionTreeLM::estimate(NgramStats &stats, Discount **discounts)
{
  return estimate2(stats, discounts);
}

Boolean
NgramDecisionTreeLM::estimate(NgramCounts<FloatCount> &stats, 
			      Discount **discounts)
{
  return estimate2(stats, discounts);
}
...
Boolean
NgramDecisionTreeLM::estimate2(NgramCounts<CountType> &stats, 
			       Discount **discounts)
{
  assert(tree);
	...
	*insertProb(vocab.ssIndex(), context) = LogP_Zero;
	...
	retry:
		...
		if (vocab.isNonEvent(word[0]) || vocab.isMetaTag(word[0])) {
		  ...
		  // Decision tree language model in action!!!
		  lprob = tree->wordProb(word[0], context);
